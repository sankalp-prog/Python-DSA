...
#   +===================================================+
    ...
    # My solution that worked first try - 
    def partition_list(self, value):
        if self.length == 0:
            return None
            
        greater_value_list = LinkedList(None)
        greater_value_list.make_empty()
        lesser_value_list = LinkedList(None)
        lesser_value_list.make_empty()
        temp = self.head
        for _ in range(self.length):
            if temp.value < value:
                lesser_value_list.append(temp.value)
            else:
                greater_value_list.append(temp.value)
                
            temp = temp.next
        self.make_empty()
        lesser_temp = lesser_value_list.head
        greater_temp = greater_value_list.head
        for _ in range(lesser_value_list.length):
            self.append(lesser_temp.value)
            lesser_temp = lesser_temp.next
        
        temp = self.head
        for _ in range(greater_value_list.length):
            self.append(greater_temp.value)
